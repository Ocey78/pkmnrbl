...
try {
    ...
    Assert-Condition -Condition ($cmakeContent -match 'else\(\)\s+target_sources\([^)]*nWiiRuntime/src/core/native_only\.cpp\)') -Message 'Generated interpreter-OFF target has no native-only fail-fast linkage.'
    Assert-Condition -Condition (Test-Path -LiteralPath (Join-Path $generatedRuntime 'src/core/native_only.cpp') -PathType Leaf) -Message 'The synthetic export did not copy its native-only fail-fast implementation.'
    ...
    Assert-Condition -Condition ($runtimeCmakeContent -notmatch 'src/core/native_only\.cpp') -Message 'Generated runtime must not force native-only guards into the interpreter-ON target.'
